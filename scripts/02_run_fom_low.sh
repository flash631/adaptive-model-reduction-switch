...
LOG="${LOG_DIR}/02_run_fom_low_${ts}.log"
...
LOW_DIR="${ROOT_DIR}/cases/low"
RESULTS_DIR="${ROOT_DIR}/results/low"
mkdir -p "${RESULTS_DIR}"

log "Syncing cases/base -> cases/low ..."
rm -rf "${LOW_DIR}"
mkdir -p "${LOW_DIR}"
if command -v rsync >/dev/null 2>&1; then
  rsync -a "${BASE_DIR}/" "${LOW_DIR}/"
else
  cp -a "${BASE_DIR}/." "${LOW_DIR}/"
fi

log "Applying low-fidelity settings (mesh/time/writeFormat)..."
...
(cd "${LOW_DIR}" && blockMesh) 2>&1 | tee -a "${LOG}"
...
if [[ -f "${LOW_DIR}/system/snappyHexMeshDict" ]]; then
  log "snappyHexMeshDict detected. By default this script does NOT run snappyHexMesh."
  log "If you want it: RUN_SNAPPY=1 bash scripts/02_run_fom_low.sh"
  if [[ "${RUN_SNAPPY:-0}" == "1" ]]; then
    log "Running snappyHexMesh..."
    (cd "${LOW_DIR}" && snappyHexMesh -overwrite) 2>&1 | tee -a "${LOG}"
  fi
fi

control_dict="${LOW_DIR}/system/controlDict"
...
log "Running solver..."
(cd "${LOW_DIR}" && "${run_cmd[@]}") 2>&1 | tee -a "${LOG}"

if command -v foamToVTK >/dev/null 2>&1; then
  # OpenFOAM-13 foamToVTK uses `-time` ranges, not `-allTime`.
  log "Running foamToVTK (all times via -time '0:') ..."
  set +e
  (cd "${LOW_DIR}" && foamToVTK -time '0:' -fields '(U p)') 2>&1 | tee -a "${LOG}"
  rc=${PIPESTATUS[0]}
  set -e
  if [[ "${rc}" -ne 0 ]]; then
...
log "Exporting snapshots to results/low/snapshots ..."
